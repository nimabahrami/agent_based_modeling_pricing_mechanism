...
class Member(Agent):
    ...
    def __init__(self, unique_id, model, member_name, agent_type, member_type, demand_flexibility, asset_list):
        ...
        #agents new behaviors
        self.anonymous = True
        self.fairness = True
        self.riskaverse = True
...
class Asset(Agent):
    ...
    def __init__(self, unique_id, model, manufacturer, capacity, efficiency, owner, asset_age,
...
class Battery(Asset):
    def __init__(self, unique_id, model, capacity=0, efficiency=0, owner=None, asset_age=1,
        ...
        super(Battery, self).__init__(unique_id, model, capacity, efficiency, owner, asset_age,
                                      estimated_lifetime_generation, capex, opex, discount_rate)
        self.autonomy_day = 3
        self.load_demand_daily = 40
        self.inverter_efficiency = 10
        self.batterey_efficiency = 10
        self.DoD = 10
        self.SoC = SoC[t-1] * (1-self.self_discharge) + self.batterey_efficiency*((p_pv_t+p_wt_t)-p_load_t/self.inverter_efficiency)
