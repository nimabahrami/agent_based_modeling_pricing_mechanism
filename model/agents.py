...
class Member(Agent):
    ...
    def calculate_cove_and_flexibility(self):
        ...
        if self.agent_type is AgentType.PROSUMER:
            ...
            # Get COVE markup from model if available, otherwise use default
            cove_markup = getattr(self.model, 'cove_markup', 1.0)
            self.cove_value = prosumer_rolling_cove(self.average_lcoe, price_data, generation_data, cove_markup)
            if not isinstance(self.cove_value, (int, float)) or self.cove_value is False:
                self.cove_value = self.average_lcoe * 1.2 * cove_markup  # Default to 20% above LCOE
        else:
            self.cove_value = 0
    ...
    def compute_savings_ToD(self):
        ...
        if self.agent_type is AgentType.PROSUMER:
            ...
            if clearing_price is not None and hasattr(self, 'bid') and self.bid is not None:
                ...
                savings_price = 0
        elif self.agent_type is AgentType.COORDINATOR:
            matched_quantity = 0
            savings_price = 0
        else:
            savings_price = 0
        ...
        self.savings_ToD = matched_quantity * savings_price
